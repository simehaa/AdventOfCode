...
def total_pressure_released(valves, flow_rates, travel_cost, minutes=26):
    N = len(valves)
    paths = [[0]]
    pressures = [0]
    ...
    for path in paths:
        ...
        for idx in valves_left:
            ...
            pressure, used_all_valves = pressure_one_path(
                new_path, flow_rates, travel_cost, minutes=minutes
            )
            if used_all_valves:
...
if __name__ == "__main__":
    ...
    paths, pressures = total_pressure_released(valves, flow_rates, travel_cost, minutes=30)
    max_pressure_human = max(pressures)
    print("Part 1:", max_pressure_human)

    # Part 2, find most pressure released by two workers
    paths, pressures = total_pressure_released(valves, flow_rates, travel_cost, minutes=26)
        ...
        for j in range(i, top_N):
            if all(valve not in paths[idx[i]][1:] for valve in paths[idx[j]][1:]):
                max_pressure_human_and_elephant = max(
                    max_pressure_human_and_elephant, pressures[idx[i]] + pressures[idx[j]]
                )
