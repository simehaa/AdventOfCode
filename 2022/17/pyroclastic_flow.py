    ...
    for current_rock in range(n_rocks):
        ...
        for j in range(len(grid)):
            ...
            if side == -1:
                for row, col in leftmost.items():
                    if col + side < 0 or grid[row][col + side] != ".":
                        can_move_sideways = False
            elif side == 1:
                for row, col in rightmost.items():
                    if col + side > 6 or grid[row][col + side] != ".":
                        can_move_sideways = False
            ...
            for row, col, rock in indices:
                if row == 0 or rock == "@" and grid[row - 1][col] != ".":
                    can_move_down = False
        ...
        if unique in uniques:
            for old_pattern in uniques[unique].copy():
                ...
                uniques[unique].append({"top_rows": grid[-15:], "rock": current_rock})
        else:
            uniques[unique] = [{"top_rows": grid[-15:], "rock": current_rock}]
